...
-export([start/2, run_simulation/1, restore/0]).
...
restore() ->
    ...
    timer:apply_after(1200, simulation, run_simulation, [{LastTime, TopTime}]).

%%run simulation from last checkpoint
run_simulation({LastTime, Lapse}) ->    
    run_simulation(Lapse, LastTime);
