...
add_car({LaneId, LanePid, WaitingOutside, ProbData, {0, _Time}}, _Car, SourcesLane, _LogPath) ->
    Arrival  =  new_arrival({LaneId, LanePid,ProbData}),
    [ {LaneId, LanePid, WaitingOutside, ProbData, Arrival} | SourcesLane ];
add_car({LaneId, LanePid, WaitingOutside, ProbData, {CarNum, Time}}, Car, SourcesLane, LogPath) when CarNum > 0 ->    
    LanePid ! {incoming, self(), Car},
    receive
        {reply, full}  ->
            io:format("Lane ~w its full. Adding to wait list~n",[LaneId]),
    ...
    add_car({LaneId, LanePid, WaitingUpdated, ProbData, {CarNum -1, Time}}, Car, SourcesLane, LogPath).
...
estimate_new_arrival([{LaneId, LanePid, [], ProbData, {CarsNum, 0}}| Tail], SourcesLane, LogPath) ->
    ...
    Car = {car,{0,0,-1}},
    UpdatedSources = add_car({LaneId, LanePid, [],  ProbData, {CarsNum, 0}}, Car, SourcesLane, LogPath),
    estimate_new_arrival(Tail, UpdatedSources, LogPath);
...
estimate_new_arrival([{LaneId, LanePid, [WaitingCar|WaitingOutside], ProbData, {CarsNum, 0}}| Tail], SourcesLane, LogPath) ->
    ...
    UpdatedSources = add_car({LaneId, LanePid, NewWaiting, ProbData, {CarsNum, 0}}, WaitingCar, SourcesLane, LogPath),
    estimate_new_arrival(Tail, UpdatedSources, LogPath);
%% If there is still missing time for a car to arrive, just update times for all cars on waiting lists
estimate_new_arrival([{LaneId, LanePid, WaitingOutside, ProbData, {CarsNum, Timer}}| Tail], SourcesLane, LogPath) ->
    io:format("NOT ADDING CAR TO LANE ~n",[]),
    write_result(LogPath, io_lib:format("NOT ADDING CAR TO LANE. LANE ~w ~n",[LaneId])),
    WaitingUpdated = waiting(WaitingOutside), 
    estimate_new_arrival(Tail, [{LaneId, LanePid, WaitingUpdated, ProbData, {CarsNum, Timer}}|SourcesLane], LogPath).
...
init_poisson([{LaneId, LanePid, [], ProbData}| Tail], SourceUpdated) ->
    Arrival = data_distribution(ProbData),
    io:format("Arrival data ~w~n",[Arrival]),
...
data_distribution(_ProbData) ->
    N = random:uniform(12),
    Lambda= 6,
    {N, lanes_poisson(N, Lambda)}.

lanes_poisson(N, Lambda) ->
    Value = statistic:poisson(N, Lambda),
    erlang:round(Value).
...
new_arrival({_LaneId, _LanePid, ProbData}) ->
 data_distribution(ProbData).
