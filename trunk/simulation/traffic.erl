...
%%Spawn FSM for traffic lights on each intersection and then connect them    
    ...
    {LightsFSM, NewSources, {NewPath,DataPath, NewCheckLogPath}}.
...
loop({Intersections, SourceLanes, {LogPath, DataLogPath, CheckPointLog}}, Time) ->
  ...
  %% Iterate intersection list and for each of them run evaluate state funciton
    ...
    {call, Pid, checkpoint} ->
    	...
    	checkpoint(Intersections, CheckPointLog),
...
traverse_network([], _Time) ->
    [];
traverse_network([{InterId, Intersection, Sequence}|Intersections], Time) ->
...
tabulate_network([], _DataLog) ->
    [];
tabulate_network([{_InterId, Intersection, Sequence}|Intersections],DataLog) ->
    %%io:format("Evaluating state for light_fsm: ~w continue.~n",[InterId]),
...
%%OUTPUT: None
checkpoint([], _CheckPointLog) ->
    [];
checkpoint([{_InterId, Intersection, Sequence} | Intersections], CheckPointLog) ->
...
%%after that get sources lanes (lanes where cars will arrive to the area)    
	...
	{LightsFile, LanesFile, CarsFile, OCarFile} = CheckPointLog,
