...
%%for each lane in the list spawn a proccess
    ...
    %io:format("~n~nMAX SPEED FOR ~w is ~w ~n~n",[LaneId, TargetMaxSpeed]),
...
set_connection_to_light_siblings([{_Id, Pid,Sequence}|LightsRem], LightsList) ->    
   %{_State, _Times, Siblings, _LogData, _OldState, _CtrlMod, _LightMode, _ManagedLanes} = invoke_light(Sequence, get_state, Pid),
   {_State, Siblings} = invoke_light(Sequence, get_state, {Pid, [siblings]}),
...
find_siblings_aux([{SiblingId, Location}|Tail], LightsList,CompleteSiblings) ->
    ...
    case SiblingData of
		...
		_Other -> {Id, Pid,Sequence} = SiblingData,
    			  ...
    			  {_State, CtrlMod} = invoke_light(Sequence, get_state, {Pid, [ctrl_mod]}),
...
light_final_format(LightsList) ->
	lists:map(fun({Id, Pid,Sequence}) ->
		%{_State, _Times, _Siblings, _LogData, _OldState, CtrlMod, _LightMode, _ManagedLanes} = invoke_light(Sequence, get_state, Pid),
		{_State, CtrlMod} = invoke_light(Sequence, get_state, {Pid, [ctrl_mod]}),
...
get_lights() ->
	file:delete("sources/test_lights_OLD.txt"),
	Sources = filemanager:get_data("/sources/test_lights.txt"),
	file:rename("sources/test_lights.txt", 
		"sources/test_lights_OLD.txt"),
	Sources.
	
get_lights([]) ->
	filemanager:get_data("/sources/test_lights.txt");
get_lights(SourceFile) ->
	filemanager:get_data(SourceFile).
	
get_lanes([]) ->
	filemanager:get_data("/sources/test_lanes.txt");
get_lanes(SourceFile) ->
	filemanager:get_data(SourceFile).

get_obs([]) ->
	filemanager:get_data("/sources/obs.txt");
get_obs(SourceFile) ->
	filemanager:get_data(SourceFile).
...
loop({Intersections, SourceLanes, {LogPath, DataLogPath, CheckPointLog}}, Time) ->
  ...
  %% Iterate intersection list and for each of them run evaluate state funciton
    ...
    {call,Pid, stop} -> 
    	safe_overwrite_sources(Intersections),
...
safe_overwrite_sources(Intersections) ->
	Sources = get_lights(),
	{ok, Cwd} = file:get_cwd(),
	FileName = Cwd ++ "/sources/lights_temp.txt",
	FileRename = Cwd ++ "/sources/test_lights.txt",
	io:format("~n~nFile ~p , Intersections ~w~n~n", [FileName, Intersections]),
	safe_overwrite_sources_aux(Intersections, Sources, FileName),
	file:rename(FileName, FileRename).	
safe_overwrite_sources_aux([], _Sources, _TargetFile) ->
	io:format("NO MORE INTERSECTIONS TO UPDATE ~n"),
    [];
safe_overwrite_sources_aux([{_InterId, InterPid, Sequence, _Sensor}|Intersections], Sources, TargetFile) ->
	io:format("TRAFFIC - UPDATING INTERSECTIONS SOURCE ~n"),
    invoke_light(Sequence, overwrite,{InterPid, Sources, TargetFile}),
    safe_overwrite_sources_aux(Intersections, Sources, TargetFile).
