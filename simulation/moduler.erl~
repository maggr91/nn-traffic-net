-module(moduler).
-export([start/0,start/1, test/0, connect/3,status/1, status_test/0, update_from_nn/2, request_updates/1, checkpoint/1, stop/1, get_sensor/1]).
...
listen(Light, Siblings, NN, Data, CheckpointLog, Sensor) ->
	receive
		...
		{proc, CallerPid, Dir} ->
			SensorInputs = sensor:get_records(Sensor),
			FormatedInputs = format_inputs(Dir, Data, SensorInputs),
			...
			update_from_nn(self(), FinalDesition),
...
format_inputs(_Dir, _Data, _SensorInputs) ->
	{}.
