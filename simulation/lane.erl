...
init({LaneId, Type, ConnectedLanes, CarsQueque, Capacity, Obstruction, ProbData}) ->    
    ...
    lane(LaneId, Type, ConnectedLanes, CarsQueque,[], Capacity, Obstruction, ProbData, [{dsp_str,0},{dsp_trn,0},{tra_sib, 0}]).

reply (Pid, Reply) ->
    Pid ! {reply, Reply}.

lane(LaneId, Type, ConnectedLanes, CarsQueque, OutSideArea, Capacity, Obstruction, ProbData, Stats) ->
  ...
  %% and run the complete simulation
    ...
    %% if a go message is received it tells all cars to start to move
	    ...
	    {NewCarsQueque, NewProbData, NewOutArea, NewStats} = move_cars(CarsQueque, ConnectedLanes,Obstruction, [], Capacity,ProbData, LaneId, OutSideArea, LogData, Stats),
            io:format("Moving msj received from ~w. NewCarsQueque: ~w ~n",[LightController,NewCarsQueque]),	    
	    reply(LightController, updated),
	    lane(LaneId, Type, ConnectedLanes, NewCarsQueque,NewOutArea, Capacity, Obstruction, NewProbData, NewStats);
    %% send a message to all cars to start to stop preventing them to pass the red ligth
	    ...
	    lane(LaneId, Type, ConnectedLanes, NewCarsQueque, OutSideArea, Capacity, Obstruction, ProbData, Stats);
	...
	{connect_lane, AdjLane, Pid}  ->     		
    	    ...
    	    reply(Pid,ok),
    	    lane(LaneId, Type, NewConnectedLanes, CarsQueque, OutSideArea, Capacity, Obstruction, ProbData, Stats);
    %% incoming msg, used when a car arrives to lane
	{incoming, Pid, {car,{_Wait, _Delay, _Position, Route, PrefLanes}}} ->
	    io:format("Car incoming~n"),
    %% set car to the las position (capacity -1)
	    %%NewCarData = {car,{Wait,Delay,Capacity - 1, Route, PrefLanes}},
	    NewCarData = {car,{0,0,Capacity - 1, Route, PrefLanes}},
	...
	{try_transfer,TransferTime, {CallerId, CallerLPid},TransferPosition, CarToTransfer, LogData} ->
	    ...
	    case space_between_cars(CarsQueque, TransferPosition, CedNumCar, CarToTransfer, Capacity - 1, TransferTime, RemCap, LogData) of
	        {true, NewCarsQueque, NewCedNumCar} -> 
	            ...
	            reply(CallerLPid, ok),
	            lane(LaneId, Type, ConnectedLanes, NewCarsQueque,OutSideArea, Capacity, Obstruction, NewProbData, NewStats);
	        {false, _SameQueque, NewCedNumCar} ->
	            ...
	            lane(LaneId, Type, ConnectedLanes, CarsQueque,OutSideArea, Capacity, Obstruction,NewProbData, Stats)
	    end;
...
move_cars([], _ConnectedLanes, _Obstruction, UpdatedCars, _LanCap, ProbData, _LaneId, NewOutArea, LogData, Stats) -> 
...
move_cars([{car,{Wait,Delay, Position, Route, PrefLanes}}|Tail], ConnectedLanes, [], UpdatedCars, LanCap, 
  ProbData, LaneId, NewOutArea, LogData, Stats) when Position - 1 >= 0 -> 
    move_cars(Tail, ConnectedLanes,[], [{car,{Wait + 1,Delay, Position - 1, Route, PrefLanes}} | UpdatedCars], LanCap, ProbData, LaneId, NewOutArea, LogData, Stats);
...
move_cars([{car,{Wait,Delay, Position, Route, PrefLanes}}|Tail], ConnectedLanes, Obs, UpdatedCars, LanCap, 
  ProbData, LaneId, NewOutArea, LogData, Stats) when Position - 1 < 0 -> 
    ...
    {Res, NewTurnCarNum, Dir} = prepare_car_dispatch({car,{Wait,Delay, Position, Route, PrefLanes}}, ConnectedLanes, Tail,TurnCarNum, {LaneId, ParentLaneId},LogData),
    ...
    %% the cars
    case Res of 
        {reply, transfered, Car}   -> move_cars(Tail, ConnectedLanes, Obs, UpdatedCars, LanCap,NewProbData, LaneId, [Car|NewOutArea], LogData, Stats);
        {reply, transfered} when Dir == str ->
        			      ...
        			      move_cars(Tail, ConnectedLanes, Obs, UpdatedCars, LanCap,NewProbData, LaneId,NewOutArea, LogData, NewStats);
...
move_cars(CarsQueque, ConnectedLanes, [ObsData | _Obstruction], _UpdatedCars, 
            _LanCap, ProbData, LaneId, NewOutArea, LogData, Stats)-> 
...
add_car({LaneId, LanePid, WaitingOutside, _ProbData, _Timer}, Car = {car,{Wait,Delay, Position, Route, PrefLanes}}, LogPath) ->    
