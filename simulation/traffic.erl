...
-export([start/1, stop/0, lane/4, set_map/0, connect/1, connect_lanes/1, test/0, write_result/2]).
...
loop({Intersections, SourceLanes}, Time) ->
  ...
  receive
      {call, Pid, continue} ->
         ...
         estimate_new_arrival(SourceLanes, 0,[]),
         %lists:foreach
         %    (
                 %fun({InterId, Intersection})->                     
                 %    io:format("Evaluating state for light_fsm: ~w continue.~n",[InterId]),
                 %    UpdatedSources = estimate_new_arrival(SourceLanes, random:uniform(100),[]),
	 ...
	 io:format("----------------------------------------------------------~n~n"),
	 loop({Intersections, SourceLanes}, Time + 1);
    {call,Pid, stop} -> 
...
lane(Type, ConnectedLanes, CarsQueque, Capacity) ->
  ...
  %% and run the complete simulation
    receive
        {go, LightController, _TimeCycle, _Time, LogData} -> %% if a go message is received it tells all cars to start to move
            io:format("GO msj~n. CarsQueque: ~w~n",[{Type, ConnectedLanes, CarsQueque}]),
	    ...
	    lane(Type, ConnectedLanes, NewCarsQueque, Capacity);
...
%	    io:format("Waiting msj~n. CarsQueque: ~w~n",[{Type, ConnectedLanes, CarsQueque}]),
	...
	{connect_lane, AdjLane, Pid}  ->     		
    	    ...
    	    reply(Pid,ok),
    	    lane(Type, NewConnectedLanes, CarsQueque, Capacity);
	{incoming, Pid, Car} ->
...
add_car({LaneId, LanePid, WaitingOutside}, Car, SourcesLane) ->    
    ...
    receive
        ...
        {reply, ok}    -> 
            ...
            [{LaneId, LanePid, WaitingOutside} | SourcesLane]
    end.
...
estimate_new_arrival([], _Prob,SourcesLane) ->
    ...
    SourcesLane;
estimate_new_arrival([{LaneId, LanePid, []}| Tail], Prob, SourcesLane) when Prob > 1  ->
...
estimate_new_arrival([{LaneId, LanePid, [WaitingCar|WaitingOutside]}| Tail], Prob, SourcesLane) when Prob > 1  ->
    io:format("ADDING CAR TO LANE ~n",[]),
    ...
    estimate_new_arrival(Tail,random:uniform(100), UpdatedSources);
estimate_new_arrival([{LaneId, LanePid, _WaitingOutside}| Tail], Prob, SourcesLane) when Prob =< 0  ->
    io:format("NOT ADDING CAR TO LANE ~n",[]),
    estimate_new_arrival(Tail,random:uniform(100), SourcesLane).
